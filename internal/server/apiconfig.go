...
// requestedCtxSize returns the context size a patch request is asking for, honouring
// both the snake_case and dashed spellings of the field (the dashed form wins, matching
// the previous write order). ok is false when the request does not set either.
func requestedCtxSize(req apicontract.ConfigModelPatchRequest) (n int, ok bool) {
	if req.CtxSize != nil {
		n, ok = *req.CtxSize, true
	}
	if req.CtxSizeDash != nil {
		n, ok = *req.CtxSizeDash, true
	}
	return n, ok
}
