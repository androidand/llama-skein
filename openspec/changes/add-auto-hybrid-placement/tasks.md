...
- [ ] 1. `internal/fit`: extend `ModelShape` (expert bytes total/per-layer,
...
- [ ] 2. `ShapeFromGGUF` populates expert bytes via
...
- [ ] 3. Unify KV math: `RecommendCpuMoe` takes a KV-bytes-per-token input
...
- [ ] 4. Descriptor/hypothetical path: per-variant verdicts gain
...
- [ ] 5. New `internal/placement`: `Plan(shape, gpuBudget, hostBudget,
...
- [ ] 6. Planner unit tests with synthetic shapes: full-GPU fit, MoE hybrid,
...
- [ ] 7. `placement:` block in `internal/config` (+ `config-schema.json` +
...
- [ ] 8. `applyAutoPlacement()` in `internal/server`, called with
...
- [ ] 9. Fit guard third remedy: before refusing/marking unfittable, consult
...
- [ ] 10. Preflight: run `llama-fit-params` (when present next to the engine
...
- [ ] 12. `contracts/llama-skein.openapi.json`: placement fields on
...
- [ ] 13. `/v1/models` runtime hints expose effective placement (sibling of
  the existing offload read-back); config add/patch responses report the
  plan.
  - Validation: `go test ./internal/server/... -run 'Models|Config'`
- [ ] 14. UI (ui-svelte): model card shows placement status ("Fits fully in
  GPU memory" / "Requires hybrid GPU + RAM" / "Runs mostly from system RAM" /
  "Insufficient safe memory"), estimated GPU/host split, expandable effective
  llama.cpp args.
...
- [ ] 16. Docs: `docs/placement.md` — how auto placement works, why file size
