...
- [ ] 1.2 **Comparability.** Define when another host's entry applies here. Establish
      ...
      rocky gfx1100 24 GB against z4). Verify: a written rule, backed by measurements.
- [ ] 1.3 **Staleness.** Measure whether an engine upgrade materially changes throughput
      for a fixed config. Re-measure muse-glimmer on rocky across two builds. Verify: data
...
- [ ] 4.4 Record peak VRAM against the run. Verify: on rocky, a captured entry reports
      ~19.7 GB for muse-glimmer at `--n-gpu-layers 99`.
...
- [ ] 5.2 `GET /api/gallery/viable` listing models runnable on this hardware.
      Verify: on rocky, muse-glimmer appears with expected throughput.
...
- [ ] 6.1 Capture entries for every model currently configured on rocky and z4, so the
...
- [ ] 6.2 Record the muse-glimmer findings explicitly: `ngl 40` → 6.1 tok/s vs `ngl 99` →
...
- [ ] 7.2 End-to-end on rocky: serve traffic, confirm an entry is captured, confirm the
      recommendation for muse-glimmer matches the measured `ngl 99` configuration.
