...
- [x] 1. In `doStart` (internal/process/process_command.go), for non-llamacpp
...
- [x] 2. Test: mock upstream whose `/health` is 200 but whose
...
- [x] 3. Verify `inferenceProbeLoop` drives a wedged-but-alive process to
  `stopped` (state stops reporting ready). Add/confirm a test.
  - Validation: `go test ./internal/process -run InferenceProbe`
- [x] 4. Review `mlxSlot` vs probe/stuck-generation: a held slot must not block
...
- [x] 5. Count repeated verified-readiness (warm-up) failures toward the
...
<!-- DEFERRED 2026-07-26: Requires live M3/M5 deploy. -->
