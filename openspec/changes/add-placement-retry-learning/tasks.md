...
- [ ] 1. `internal/process`: add `FailureClass` (gpu-oom, host-oom,
...
- [ ] 2. Fixture-driven parser tests: ROCm GPU OOM, CUDA OOM, host cgroup
...
- [ ] 3. `internal/placement`: `NextSaferPlan(prev Plan, class FailureClass)
...
- [ ] 4. Supervisor wiring: on memory-class failure of an auto-placed model,
...
- [ ] 5. Contract: ladder/attempt history + failure class in the OpenAPI
  model-status shapes; regenerate.
  - Validation: `go generate ./pkg/apicontract && make check-codegen`

## Phase 3 — Learned profiles
- [ ] 6. `internal/placement/profilestore`: JSON persistence keyed by model
...
- [ ] 7. Measure peaks during load/warm-up from existing perf samples; write
...
- [ ] 8. Planner consults the store first; invalidation on any key mismatch;
  fail-open to fresh planning.
  - Validation: `go test ./internal/placement/...`

## Phase 4 — Gate + acceptance
- [ ] 9. `go build ./... && go test -short ./... && make test-dev`.
