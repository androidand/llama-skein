...
- [x] 1. Spec: in `contracts/llama-skein.openapi.json` add `backend_env`
...
- [x] 2. Database: add a top-level `backend_env.linux_glibc` map to
...
- [x] 3. Config/override: add `BackendEnv *bool` (`yaml:"backend_env"`) to
       `config.TuningConfig` and to `tuning.Override`; map it in `ToOverride`.
       Validation: `go build ./internal/config/ ./internal/tuning/`

- [x] 4. Inject: add `Database.BackendEnvFor(tc)` returning the effective env
...
- [x] 5. API: surface `backend_env` in `buildTuningStatus`; handle
       `backend_env` in `patchToConfig` + `writeTuningToConfig`.
       Validation: `go test ./internal/server/ -run Tuning`

- [x] 6. Entrypoint: export the two `MALLOC_*` vars in
       `docker-entrypoint.sh` before `exec llama-skein`.

- [x] 7. Repo validation: gofmt, `go build ./...`, `make test-dev`,
       `make check-codegen`.

- [x] 8. (companion, opencode repo) regen the TS client
...
- [x] 9. Deploy: roll the new binary + entrypoint to the Linux GPU hosts
